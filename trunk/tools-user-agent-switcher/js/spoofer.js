...
function getOptions() {
	...
	if (options_list[i]) options_list[i].index = i;
	return options_list;
}
...
function addCustomUAOption(name, user_agent, append_to_default_ua, indicator, group) {
	var custom_options_list = getCustomOptions();
	if (!custom_options_list) custom_options_list = new Array();

	// Check that the name is not already taken.
	for (var i = 0; i < custom_options_list.length; i++)
	if (custom_options_list[i].title == name) return false;
...
function deleteUAOption(name, is_base_option) {
	...
	if (is_base_option) setBaseOptionsList(options_list)
	else setCustomOptions(options_list);
...
function getUserAgentGroup(user_agent) {
	...
	if (user_agent.group) return user_agent.group;
	return "";
}
