...
function renderService(name, lowercaseName, blocked, blockedCount, control, badge, text) {
	var imageSrcPrefix = 'images/';
	if (blocked) {
		if (blockedCount) {
			badge.src = imageSrcPrefix + lowercaseName + '-blocked.png';
			text.removeAttribute('class');
		} else {
			badge.src = imageSrcPrefix + lowercaseName + '-activated.png';
			text.className = 'activated';
		}

		control.title = 'Unblock ' + name;
	} else {
		badge.src = imageSrcPrefix + lowercaseName + '-deactivated.png';
...
onload = function () {
	chrome.tabs.getSelected(null, function (tab) {
		...
		TAB_BLOCKED_COUNTS ? TAB_BLOCKED_COUNTS[1] : BACKGROUND.initializeArray(SERVICE_COUNT, 0);
		...
		for (var i = 0; i < SERVICE_COUNT; i++) {
			...
			var control = SURFACE.appendChild(SURFACE.getElementsByTagName('tr')[0].cloneNode(true));
			var badge = control.getElementsByTagName('img')[0];
			var text = control.getElementsByTagName('td')[1];
			renderService(name, lowercaseName, DESERIALIZE(localStorage[blockedName]), blockedCount, control, badge, text);
			badge.alt = name;
			...
			control.onclick = function (service, name, lowercaseName, blockedName, blockedCount, control, badge, text) {
				const URL = service[4];
				const BLOCKED = localStorage[blockedName] = !DESERIALIZE(localStorage[blockedName]);

				if (DESERIALIZE(localStorage.searchDepersonalized) && URL) {
					if (BLOCKED) {
						BACKGROUND.mapCookies(URL, service);
					} else {
						BACKGROUND.reduceCookies(URL, service);
					}
				}
			...
			null, service, name, lowercaseName, blockedName, blockedCount, control, badge, text);
		}
