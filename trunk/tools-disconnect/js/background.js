...
function deleteCookies(url, domain, path, storeId, name) {
	...
	COOKIES.getAll(DETAILS, function (cookies) {
		...
		for (var i = 0; i < COOKIE_COUNT; i++) {
			...
			if (cookie.domain == domain && cookie.path == path) COOKIES.remove({
				url: url,
...
function incrementCounter(tabId, serviceIndex) {
	...
	BLOCKED_COUNTS[tabId] || (BLOCKED_COUNTS[tabId] = [0, initializeArray(SERVICE_COUNT, 0)]);
...
const SERVICES = [
	...
	'avatars',
	'help',
	'login', // "login" is required for OpenID access but conflicts with "edit".'messages',
	'pipes',
	'realestate',
	'smallbusiness',
	'travel',
...
for (i = 0; i < SERVICE_COUNT; i++) {
	...
	url && deserialize(localStorage[service[0].toLowerCase() + BLOCKED_NAME])) {
		...
		if (deserialize(localStorage.searchDepersonalized)) mapCookies(url, service);
	}
}
...
chrome.extension.onRequest.addListener(function (request, sender, sendResponse) {
	if (request.initialized) {
		...
		for (var i = 0; i < SERVICE_COUNT; i++) {
			var service = SERVICES[i];
			BLACKLIST[i] =
			deserialize(localStorage[service[0].toLowerCase() + BLOCKED_NAME]) ? [service[1], !! service[2]] : [[]];
		}
...
COOKIES.onChanged.addListener(function (changeInfo) {
	if (deserialize(localStorage.searchDepersonalized) && !changeInfo.removed) {
		...
		for (var i = 0; i < SERVICE_COUNT; i++) {
			...
			if (
			url && deserialize(localStorage[service[0].toLowerCase() + BLOCKED_NAME]) && DOMAIN == domain && PATH == '/' && NAME != 'AO') {
				// The cookie API doesn't properly expire cookies.
				if (!EXPIRATION || EXPIRATION > TIMESTAMP() / 1000) mapCookie(
				COOKIE, COOKIE.storeId, url, domain, service[2], service[3]);
				else reduceCookies(url, service, NAME);
				if (START_TIME <= TIMESTAMP() - 3000) setTimeout(function (serviceIndex) {
