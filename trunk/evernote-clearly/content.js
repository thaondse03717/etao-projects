...
chrome.extension.onRequest.addListener(function (request, sender, sendResponse) {
	...
	case 'to-content--evernote-login-show':
		...
		__custom_events__names_to_objects['to-browser--evernote-login-show'], window.document, window);
		break;

	case 'to-content--evernote-login-failed':
		__custom_events__dispatch(
		__custom_events__names_to_objects['to-browser--evernote-login-failed'], window.document, window);
		break;

	case 'to-content--evernote-login-failed--username':
		__custom_events__dispatch(
		__custom_events__names_to_objects['to-browser--evernote-login-failed--username'], window.document, window);
		break;

	case 'to-content--evernote-login-failed--password':
		__custom_events__dispatch(
		__custom_events__names_to_objects['to-browser--evernote-login-failed--password'], window.document, window);
		break;


	case 'to-content--evernote-login-successful':
		__custom_events__dispatch(
		__custom_events__names_to_objects['to-browser--evernote-login-successful'], window.document, window);
		break;

	case 'to-content--evernote-clip-successful':
		__custom_events__dispatch(
		__custom_events__names_to_objects['to-browser--evernote-clip-successful'], window.document, window);
		break;

	case 'to-content--evernote-clip-failed':
		__custom_events__dispatch(
		__custom_events__names_to_objects['to-browser--evernote-clip-failed'], window.document, window);
		break;
	}
...
window.document.addEventListener('click', function (_event) {
	...
	case 'to-extension--open-settings':
		chrome.extension.sendRequest({
			_type: 'to-chrome--open-settings'
		});
		_stop = true;
		break;
	case 'to-extension--open-settings-theme':
		chrome.extension.sendRequest({
			_type: 'to-chrome--open-settings-theme'
		});
		_stop = true;
		break;

	case 'to-extension--select-theme-1':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-theme-1'
		});
		_stop = true;
		break;
	case 'to-extension--select-theme-2':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-theme-2'
		});
		_stop = true;
		break;
	case 'to-extension--select-theme-3':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-theme-3'
		});
		_stop = true;
		break;

	case 'to-extension--select-theme-custom':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-theme-custom'
		});
		_stop = true;
		break;

	case 'to-extension--select-size-small':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-size-small'
		});
		_stop = true;
		break;
	case 'to-extension--select-size-medium':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-size-medium'
		});
		_stop = true;
		break;
	case 'to-extension--select-size-large':
		chrome.extension.sendRequest({
			_type: 'to-chrome--select-size-large'
		});
		_stop = true;
		break;
	...
	case 'to-extension--track--theme-popup':
		chrome.extension.sendRequest({
			_type: 'to-chrome--track--theme-popup'
		});
		_stop = true;
		break;
	...
	case 'to-extension--track--settings':
		chrome.extension.sendRequest({
			_type: 'to-chrome--track--settings'
		});
		_stop = true;
		break;
