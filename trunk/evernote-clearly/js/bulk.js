$(function ()
{
			...
			$R.debugOutline = function (_element, _category, _reason)
			{
				switch (true)
				{
					...
					default:
						...
						switch (true)
						{
							case (_category == 'target' && _reason == 'first'): 				_color = '#00cc00';	break;
							case (_category == 'target' && _reason == 'second'): 				_color = '#0000cc';	break;
    ...
    //  =============
        ...
        +	'<div id="fitts" title="' + __escapeForHTML($R.translate('fitts__tooltip')) + '"></div>'
        ...
        +						'<input type="button" class="floatingButton dialogButton theFont" id="evernote_login__button_do" tabindex="4" value="' + __escapeForHTML($R.translate('evernote_login__button_do__label')) + '"/>'
        +						'<a id="evernote_login__register" class="floatingButton" href="https://www.evernote.com/Registration.action?code=clearly" target="_blank">' + __escapeForHTML($R.translate('evernote_login__create_account')) + '</a>'
        +					'</div>'
        ...
        +						'<div class="themeTitle">' + __escapeForHTML($R.translate('settings__theme__1')) + '</div>'
        +					'</a>'
        +					'<a id="settings__4__2" class="themeBox">'
        +						'<div class="themeThumbnail"></div>'
        +						'<div class="themeTitle">' + __escapeForHTML($R.translate('settings__theme__2')) + '</div>'
        +					'</a>'
        +					'<a id="settings__4__3" class="themeBox">'
        +						'<div class="themeThumbnail"></div>'
        +						'<div class="themeTitle">' + __escapeForHTML($R.translate('settings__theme__3')) + '</div>'
        +					'</a>'
	...
	//	======
		...
		$('#settings__4__custom').click(function ()
		{
			//	show settings
				...
				{
					$R.customEvents.dispatch(
						$R.customEvents.names_to_objects['to-extension--open-settings-theme'], 
						$R.document, 
						$R.win
					);
	...
	$R.menu_functions['settings'] = function ()
	{
        ...
        //  ===========
            $R.customEvents.dispatch(
				$R.customEvents.names_to_objects['to-extension--track--theme-popup'], 
				$R.document, 
				$R.win
			);
	...
	$R.getContent__findInPage = function (_pageWindow)
	{
		...
		//	============
			...
			if ($R.debug)
			{
                ...
                $(_firstCandidate.__node).css({'background-color': 'rgba(0, 255, 0, 0.25)'});
            }
			...
			switch (true)
			{
				...
				default:
                    ...
                    if ($R.debug)
                    {
                        ...
                        $(_secondCandidate.__node).css({'background-color': 'rgba(0, 0, 255, 0.25)'});
                    }
				...
				$R.show(function ()
				{
                    ...
                    //  ===========
                            ...
                            if ($R.trackedView); else
                            {
                                ...
                                //  trigger
                                $R.customEvents.dispatch(
                                    $R.customEvents.names_to_objects['to-extension--track--view'], 
                                    $R.document, 
                                    $R.win
                                );
                            }
